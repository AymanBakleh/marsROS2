...
class TetrixGoalCommander(Node):
    def __init__(self) -> None:
        super().__init__("tetrix_goal_commander")

        self.declare_parameter("goal_topic", "/tetrix/goal")
        self.declare_parameter("world_name", "default")
        self.declare_parameter("entity_name", "tetrix_robot")
        ...
        self.get_logger().info("Tetrix goal commander is ready.")
    ...
    def goal_callback(self, msg: Point) -> None:
        ...
        self.get_logger().info(
            f"Received Tetrix goal: x={msg.x:.2f}, y={msg.y:.2f}, z={msg.z:.2f}"
        )
    ...
    def control_loop(self) -> None:
        if self.pending_future is not None:
            ...
            if response is None or not response.success:
                self.get_logger().warn("Gazebo rejected Tetrix pose update.")
                return
        ...
        # Keep Tetrix on its current z unless a new z is explicitly given.
        target_z = self.current_pose.position.z
        if abs(self.active_goal.z) > 1e-6:
            target_z = self.active_goal.z
        dz = target_z - self.current_pose.position.z

        distance = math.sqrt(dx * dx + dy * dy + dz * dz)
        if distance < self.goal_tolerance:
            self.goal_active = False
            self.get_logger().info("Tetrix goal reached.")
            return

        step = min(self.max_speed * self.control_period, distance)
        scale = step / distance if distance > 1e-6 else 0.0
